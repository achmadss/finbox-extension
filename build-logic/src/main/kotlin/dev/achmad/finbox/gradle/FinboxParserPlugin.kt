...
/**
 * Builds a finbox parser APK:
 * - Android application module, no launcher activity
 * - Manifest declares the `dev.achmad.finbox.parser` feature plus
 *   `finbox.parser.lib` metadata and a fixed `finbox.parser.class`,
 ...
 */
private const val GENERATED_CLASS = "dev.achmad.finbox.parser.generated.GeneratedSource"

class FinboxParserPlugin : Plugin<Project> {
    override fun apply(target: Project) {
        with(target) {
            ...
            val finbox = extensions.create<FinboxParser>("finbox")
            ...
            configure<ApplicationExtension> {
                ...
                defaultConfig {
                    minSdk = 26
                    applicationId = "dev.achmad.finbox.parser.${project.name}"
                }
                testOptions {
    ...
    private fun manifest(finbox: FinboxParser, apiVersion: String): String = """
        <?xml version="1.0" encoding="utf-8"?>
        <manifest xmlns:android="http://schemas.android.com/apk/res/android">
            <uses-feature
                android:name="dev.achmad.finbox.parser"
                android:required="true" />
            <application android:icon="@mipmap/ic_launcher">
                <meta-data
                    android:name="finbox.parser.lib"
                    android:value="$apiVersion" />
                <meta-data
                    android:name="finbox.parser.class"
                    android:value="$GENERATED_CLASS" />
                <meta-data
                    android:name="finbox.parser.provider"
                    android:value="${finbox.provider}" />
                <meta-data
                    android:name="finbox.parser.name"
